...
#define KP   (float) 0.95f //1.1f //0.8f //0.9f//0.7f //1.0f //0.751
#define KI   (float) -0.4f //-0.35f //-0.4f//-0.035f//-0.8f
#define KD   (float) -0.3f //-0.2f //-0.2f//-0.11f//-0.125f 0.07
#define	TD   (float) 0.1f//0.1f
#define TP   (float) 0.01428571f
#define TRES (float) 0.0f
#define TF   (float) 0.1428571f 
...
extern float fuzzy_controller(float input1, float input2);
/*
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	...
	if(htim->Instance == TIM10){ //70Hz Interrupt for sending data to PC
	  ...
	  //MPU6050_GetGyroscopeScaled(&gx, &gy, &gz);
		...
		size = sprintf(send_data, "%.2f,%.2f,%d,%d,%d,%.2f \n", roll_comp, pitch_comp, TIM2->CCR2, TIM3->CCR1, controller_type, sphereRoll_SetPoint);
		HAL_UART_Transmit_DMA(&huart1, send_data, size);

	}
	
	if(htim->Instance == TIM13) { // 200Hz interrupt for CONTROLLERS
		/*
		...
		*/
		...
		/* CALCULATE CONTROL*/
		if(controller_type == 1){  //if controller is PID
			control = (roll_error*wspE + prev_roll_error*wspE1 + prev_prev_roll_error*wspE2 + prev_control*wspU1 + prev_prev_control*wspU2)/TD;
		}
		else if(controller_type == 2){ //if controller is Fuzzy
			error_derivative = (roll_error - prev_roll_error - (TP-0.15)*error_derivative)/0.15;
			if(error_derivative > 120){
				error_derivative = 120;
			}
			else if(error_derivative <-120){
				error_derivative = -120;
			}
			
			control = -fuzzy_controller(roll_error, error_derivative);
		}
		else{
			control = 0;
		}
		...
		/* CHECK IF ERROR DOES NOT EXCEED THE LIMITS */
		if(control>99)
			 control = 99;
		else if(control<-99)
			 control = -99;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	...
	*/
	...
	/* Check if controller type is not being changed*/
	if(servo == 'P' || servo == 'p' ){
		controller_type = 1; // Change type of controller PID  and terminate callback function
		HAL_UART_Receive_DMA(&huart1, received_data, 4);
		return;
	}else if(servo == 'F' || servo == 'f'){
		controller_type = 2; //Change controller to Fuzzy and terminate callback function
		HAL_UART_Receive_DMA(&huart1, received_data, 4);
		return;
	}else if(servo == 'N' || servo == 'n'){
		controller_type = 0; //Change controller to Fuzzy and terminate callback function
		HAL_UART_Receive_DMA(&huart1, received_data, 4);
		return;
	}
	/* END of check if controller type is not being changed*/
