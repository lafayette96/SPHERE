...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	...
	if(htim->Instance == TIM10){ //70Hz Interrupt for sending data to PC
		...
		magd_roll  = atan2f(2*q1*q3 - 2*q0*q2, 1 - 2*q1*q1 - 2*q2*q2);
		magd_pitch = atan2f(2*q0*q3 - 2*q1*q2, 1 - 2*q0*q0 - 2*q2*q2);

		
		size = sprintf(send_data, "%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%d \n", roll, pitch, MPU6050.KalmanAngleX, MPU6050.KalmanAngleY,magd_roll, magd_pitch, TIM2->CCR2);
