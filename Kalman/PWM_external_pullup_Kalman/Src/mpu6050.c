...
void ComplementaryFilter(MPU6050_t *DataStruct, float* roll, float* pitch)
{
	...
	gyr_x = DataStruct->Gyro_X_RAW / 131.0;
	gyr_y = DataStruct->Gyro_Y_RAW / 131.0;
	gyr_z = DataStruct->Gyro_Z_RAW / 131.0;
	
	acc_x_raw = DataStruct->Accel_X_RAW;
	acc_y_raw = DataStruct->Accel_Y_RAW;
	acc_z_raw = DataStruct->Accel_Z_RAW;
  ...
  // Sensitivity = -2 to 2 G at 16Bit -> 2G = 32768 && 0.5G = 8192
	...
	if(forceMagnitudeApprox > 8192 && forceMagnitudeApprox < 32768)		// 8192 = (0.5*(1/Acc_Scale)) 32768 = 4*8192
	{
		...
		pitchAcc = atan2f((float)acc_y_raw, (float)acc_z_raw) * RAD_TO_DEG; 		//atan2f(floats) is a sigle precision version of atan2(doubles)
		*pitch = (float)*pitch * 0.978 + pitchAcc * 0.022;

		// Turning around the Y axis results in a vector on the X-axis
		rollAcc = atan2f((float)acc_x_raw, (float)acc_z_raw) * RAD_TO_DEG;
		*roll = (float)*roll * 0.978 + rollAcc * 0.022;
	}
}
